(function($) {
	...
	$.fn.coinslider= $.fn.CoinSlider = function(options){
		...
		$.transition = function(el,direction){
			...
			if($.browser.msie&&parseInt($.browser.version,10)<=8){
				$('.banner').animate({left:-340},function(){$('.banner').eq(imagePos[el.id]).css({left:70}).find('>strong').eq(0).css({top:-50}).next().css({bottom:-50}).prev().animate({top:179},600,'easeOutExpo').next().delay(150).animate({bottom:178},600,'easeOutExpo')})
			}else{
				$('.banner').animate({opacity:0},function(){$('.banner').eq(imagePos[el.id]).css({opacity:1}).find('>strong').eq(0).css({top:-50}).next().css({bottom:-50}).prev().animate({top:179},600,'easeOutExpo').next().delay(150).animate({bottom:178},600,'easeOutExpo')})
			}
		};
